...
if ~isempty(referenceNet)
    ...
    for i = 1:numel(net.layers)-1
        if (isa(net.layers(i).block, 'dagnn.Conv') && net.layers(i).block.hasBias)
            ...
            if i<1
                net.params(bias).value = net.params(bias).value + 0.1*std(net.params(bias).value(:)) * randn(size(net.params(bias).value),'single');
                net.params(filt).value = net.params(filt).value + 0.1*std(net.params(filt).value(:)) * randn(size(net.params(filt).value),'single');
            else
                net.params(bias).value = rand(size(net.params(bias).value),'single') + 0.5;
                net.params(filt).value = initWeightsStd * randn(size(net.params(filt).value),'single');
            end
        end
    end
    filters = single(bilinear_u(16, 2, 2)) ;
    p = net.getParamIndex(net.layers(43).params{1});
